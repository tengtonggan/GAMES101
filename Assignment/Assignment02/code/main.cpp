...
Eigen::Matrix4f get_projection_matrix(float eye_fov, float aspect_ratio, float zNear, float zFar)
{
    ...
    auto l = -r;
    projection << 2.0 * sign_n / (r - l), 0, (l + r) / (l - r), 0,
        0, 2.0 * sign_n / (t - b), (b + t) / (b - t), 0,
...
int main(int argc, const char** argv)
{
    ...
    while(key != 27)
    {
        ...
        //auto& depth = r.depth_buffer();
        //std::ofstream  ofs; //创建流对象 
        //ofs.open("depth_map.txt", std::ios::out);//3.打开文件
        ...
        //}
        //ofs.close();//4.关闭文件
        cv::Mat depth_image(700, 700, CV_32FC1, r.depth_buffer().data());
        depth_image.convertTo(depth_image, CV_8UC1, 1.0f);
